...
async def send_news():
    try:
        ...
        users = cur.fetchall()
        cur.execute(
            "SELECT title, content, image FROM news ORDER BY created_at DESC LIMIT 1"
        )
        data = cur.fetchall()
        for user in users:
            for news in data:
                ...
                if news["image"]:
                    try:
                        rasm = os.path.join(
                            os.path.dirname(__file__), f"../media/{news['image']}"
                        )
                    ...
                    await bot.send_message(
                        user["telegram_id"], caption, parse_mode="Markdown"
                    )
    except BotBlocked:
